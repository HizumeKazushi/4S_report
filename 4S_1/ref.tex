% !TEX root = ./main.tex

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{center}
%\section*{参\,考\,文\,献}                      %% ここに番号をつけない
%\end{center}
%\addcontentsline{toc}{section}{参考文献} %% 目次に番号をつけない
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{thebibliography}{99}
\bibitem{Shima1}
\label{cite:Shima1}
	島ほか：非線形システム制御論，
	コロナ社 (1997)

\bibitem{Kawata1}
\label{cite:Kawata1}
	川田，島津，井上：
	Hamilton-Jacobi 方程式に基づく非線形 \(\mathcal{H}_{\infty}\) 制御の近似実現，
	システム制御情報学会論文誌，
	Vol.\,11，No.\,7，pp.\,401--410 (1998)

\bibitem{Schaft1}
\label{cite:Schaft1}
	A. J. van der Schaft: 
	\(\mathcal{L}_{\textrm{2}}\)-gain Analysis of Nonlinear Systems and 
		Nonlinear State Feedback \(\mathcal{H}_{\infty}\) Control, 
	\textit{IEEE Trans.\,Automat.\,Contr.}, 
	Vol.\,AC-37, No.\,6, pp.\,770--784 (1992)

\bibitem{Nakamura1}
\label{cite:Nakamura1}
	中村：
	二次安定化による倒立振子システムのロバスト制御に関する研究，
	立命館大学理工学部卒業論文 (1997)

\end{thebibliography}
% *******************************************